...
def estimate_poses(corners, ids, dict_names, camera_matrix, dist_coeffs, marker_size,
                   z_range_min=0.05, z_range_max=2.0, talk=True, prev_rvecs=None):
    ...
    Args:
        ...
        marker_size: Physical marker size in meters (the detectable black square)
    ...
    for corner, marker_id, dict_name in zip(corners, ids, dict_names):
        ...
        # Always use IPPE solution 0 (lowest reprojection error)
        rvec = rvecs[0]
        tvec = tvecs[0]
        tvec_flat = tvec.flatten()

        # Z-range validation: reject markers behind camera or too far
        if tvec_flat[2] < z_range_min or tvec_flat[2] > z_range_max:
            continue
        ...
        # Update prev_rvecs for next frame
        if prev_rvecs is not None:
            prev_rvecs[key] = rvec.copy()
